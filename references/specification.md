...
### 7.3 治理评审核验方法论 (Governance Review Verification Methodology)

本节固化 TERMBASE_GAP 提案入库评审与存量词条周期复核的核验流程；检索导航入口见 [`authority-index.md`](authority-index.md)。

**核验试金石 — 可引用文书测试**：MANDATORY 的判定标准不是"表述听起来官方"，而是**存在一份可引用的权威文书**（法案、监管通告、计划规则、法定计划条款）对表述构成强制约束。评审结论必须落到具体文书，不依赖评审者口头断言。

**核验五步**：

1. **定辖权** — 按词条 `scope` 与界面目标市场锁定 `authority-index.md` 对应市场区块；GLOBAL 词条先查国际标准区块。
2. **检索一手来源** — 只查官方发布渠道（监管方站点、法定文书库、系统运营方规则页），不采信二级转述（媒体报道、博客、百科）。
3. **定位具体文书** — 在文书族内确认该术语被定义或强制，记录文书名、条款位置与文书页 URL。
4. **交叉验证** — 至少两个独立来源互证（监管方 + 运营方，或法规库 + 机构既有合规意见）；单一来源存疑时按决策树降级。
5. **落库** — 合规级、`source_authority`、文书级 `source_url`、`review_date`（核验当日）写入 `termbase.csv`，过门禁后生效。

**判定决策树**：

| 证据强度 | 结论 | 落库动作 |
| :--- | :--- | :--- |
| 法定文书强制该措辞（法案 / 通告 / 计划规则条款） | MANDATORY | 必须带文书级 `source_url`；口径字字对齐，不得改写 |
| 仅 ISO / 卡组织 / 行业标准背书 | STANDARD | 带标准目录或运营方链接 |
| 仅品牌方规范（商标 / 拼写要求），无监管约束 | STANDARD | `source_authority` 写品牌方；不构成法定锁定 |
| 查无任何背书，仅为通用表达 | GENERAL | 或暂缓入库（保留提案与核验线索） |

**评审纪律**：

- **举证责任倒置**：主张 MANDATORY 就必须交出文书级 URL；交不出，只能按 STANDARD / GENERAL 入库。新入库与到期复核时，`source_url` 须指向具体文书页面——站点首页或导航入口页不合格；存量首页级链接在下次到期复核时升级为文书级。
- **宁降勿升**：误标 MANDATORY 会把本可本地化的文案锁死（法定词严禁改写），漏标可经复核周期纠正。存疑时先按较低层级入库并在 `note` 留核验线索，取得文书证据后再升级。
- **周期纠偏**：MANDATORY 词条超过 `governance.mandatory_max_age_days` 未复核即阻断发布；到期复核时同步复查合规级分类是否仍成立（监管文书可能废止或改写）。

**索引边界**：`authority-index.md` 只承载入口级导航（机构、文书族、入口 URL），不收录文书正文——正文受版权与修订时效约束，静态副本会脱离权威来源静默过期。文书级深链只存在于 `termbase.csv` 的 `source_url`，随词条生命周期治理。
